...
private:
	...
	std::shared_ptr<UA_Server> server;
...
UA_Boolean RiaOpcAdapter::running;


RiaOpcAdapter::RiaOpcAdapter()
{
...
void RiaOpcAdapter::SignalHandler(int sig)
{
	running = false;
}


void RiaOpcAdapter::FillVariables()
{
	...
	// 3) Add the node 
	UA_Server_addVariableNode(server.get(), newNodeId, parentNodeId, parentReferenceNodeId,
		browseName, variableType, attr, NULL, NULL);
}


void RiaOpcAdapter::Run()
{
	...
	std::shared_ptr<UA_ServerConfig> config(UA_ServerConfig_new_default(), UA_ServerConfig_delete);
	//config->
	server = std::shared_ptr<UA_Server>(UA_Server_new(config.get()), UA_Server_delete);
	...
	// Run the server loop 
	running = true;
	status = UA_Server_run(server.get(), &running);
